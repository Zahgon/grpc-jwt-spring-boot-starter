...
public class AuthServerInterceptor implements ServerInterceptor {
    ...
    public AuthServerInterceptor(AllowedCollector allowedCollector, JwtService jwtService, Environment environment) {
    ...
    private <ReqT, RespT> ForwardingServerCallListener<ReqT> buildListener(ServerCall<ReqT, RespT> call, Metadata metadata, ServerCallHandler<ReqT, RespT> next, Context context, JwtContextData contextData) {
    ...
    private boolean validateExposedAnnotation(JwtContextData contextData, String methodName) {
        ...
        final boolean methodIsExposed = Arrays.stream(environment.getActiveProfiles()).anyMatch(exposedToEnvironments::contains);
        if (methodIsExposed) {
            if (contextData == null)
                throw new AuthException("Missing JWT data.");
            final List<String> rawEnvironments = (List<String>) contextData.getJwtClaims().get(JwtService.TOKEN_ENV, List.class);
    ...
    private <ReqT> void validateAllowedAnnotation(ReqT request, JwtContextData contextData, String methodName) {
        allowedCollector.getAllowedAuth(methodName).ifPresent(value -> authorizeOwnerOrRoles(request, contextData, value));
    }
    ...
    private <ReqT> void authorizeOwner(ReqT request, JwtContextData jwtContext, AllowedMethod allowedMethod) {
        ...
        if (!jwtContext.getUserId().equals(uid))
            validateRoles(new HashSet<>(allowedMethod.getRoles()), jwtContext.getRoles());
    }
